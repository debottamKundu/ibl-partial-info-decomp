...
from ibl_info.measures.broja_pid import (
    compute_pid,
    coinformation,
    compute_pid_unbiased,
    unbiasedMI,
    MI,
)
...
def clean_up_neurons(
    data,
    params,
    region_names,
    percent_of_no_spikes_threshold=0.3,
...
def decode_epoch(
    ...
    for region_idx in range(len(actual_regions)):
        ...
        if normalization_params["drop"]:
            data_discretized = discretize_neural_data(
                data_epoch[region_idx], normalization_params["normalization"]
            )
        else:
            data_discretized = discretize_neural_data(
                data_epoch[region_idx].T, normalization_params["normalization"]
            )  # TODO: clean this up as well, signature should be the same
...
def decompose_epoch(
    ...
    if normalization_params["drop"] == True:
        ...
        data_epoch, kept_regions, firing_rates = clean_up_neurons(
            data_epoch, temp_params, actual_regions, plot_flag=plot_flag
        )
    ...
    if group and len(actual_regions) > 0:
        ...
        # no Mi corrections
        data_discretized = discretize_neural_data(
            data_epoch, normalization_params["normalization"]
        )
    ...
    for region_idx in range(len(actual_regions)):
        ...
        if normalization_params["mi_calculation"] == True:
            ...
            pid_info, coinfo = compute_information_decomposition(
                decoding_variable, data_discretized
            )
...
def partial_decomposition(
    one, session_id, list_of_regions, group=False, normalization_params=None
):
    if normalization_params == None:
        normalization_params = {
            "run_correction": "qe",
...
if __name__ == "__main__":
    ...
    list_of_eids = np.load(
        "D:\\personal\\phD\\code\\information-decomposition\\ibl-partial-info-decomp\\data\\processed\\eids_with_detailed_insertions_v3.npy",
        allow_pickle=True,
    )
    ...
    # setup done
    list_of_regions = {}
    list_of_regions["stim"] = np.asarray(
        ["VISpm", "PRNc", "IP", "VISli", "VM", "SCm", "GRN", "LGv", "VISam", "PB"]
    )
    list_of_regions["choice"] = np.asarray(
        ["PRNc", "GRN", "VM", "IP", "APN", "PPN", "VISpm", "GPe", "DCO", "PL"]
    )
    list_of_regions["feedback"] = np.asarray(
        ["PPN", "AUDp", "PAG", "PRNc", "IC", "AUDv", "GRN", "IP", "MRN", "RN"]
    )
